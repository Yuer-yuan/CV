...
//    if (double_size) {
//        double sig_diff = std::sqrt(sigma * sigma - 0.5f * 0.5f * 4);
//        int ksize = 2 * cvRound(3.0f * sig_diff) + 1;
//        cv::Mat dbl;
...
//    } else {
//        double sig_diff = std::sqrt(sigma * sigma - 0.5f * 0.5f * 1);
//        int ksize = 2 * cvRound(3.0f * sig_diff) + 1;
...
//					Size(gauss_pyramid[i - 1][nOctaveLayers].cols / 2,
//					gauss_pyramid[i - 1][nOctaveLayers].rows / 2), 0, 0, INTER_LINEAR);
//			}
//			else
//			{
//				//高斯滤波窗口大小选择很重要，这里选择(4*sig_diff_1+1)-(6*sig_diff+1)之间
    ...
    for (int o = 0; o < nOctaves; o++) {
        for (int i = 0; i < nOctaveLayers + 3; i++) {
            ...
            if (o == 0 && i == 0) {
                ...
                int ksize = 2 * cvRound(3.0f * sig[i]) + 1;
...
//	}
//}
void MySift::build_dog_pyramid(std::vector<std::vector<cv::Mat>> &dogpyr, const std::vector<std::vector<cv::Mat>> &gpyr) const {
...
/*image表示特征点所在位置的高斯图像
 ...
 hist表示计算得到的直方图
 函数返回值是直方图hist中的最大数值*/
static float calc_orientation_hist(const Mat &image, Point pt, float scale, int n, float *hist)
{
...
//static float calc_orientation_hist(const cv::Mat &img, cv::Point pt, float scale, int n, float *ori_hist) {
//    int radius = cvRound(4.5f * scale);
//    int len = (radius * 2 + 1) * (radius * 2 + 1);
//    float sig = 1.5f * scale;
//    float expf_scale = -1.f / (2.f * 1.6f * 1.6f);
//    cv::AutoBuffer<float> buf(len * 5 + 36 + 4);    // assign `nbins + 4` to `temphist` to soft hist
...
//    cv::hal::magnitude32f(X, Y, Mag, len);
//    for (int i = 0; i < len; i++) {
//        int bin = cvRound(((float)36 / 360.f) * Ori[i]);
//        if (bin >= 36) {
//            bin -= 36;
//        }
//        if (bin < 0) {
...
//    for (int i = 1; i < 36; i++) {
//        ori_max = ori_max > ori_hist[i] ? ori_max : ori_hist[i];
//    }
//    return ori_max;
//}
...
//static bool adjust_local_extrema(const vector<vector<Mat>> &dog_pyr, KeyPoint &kpt, int octave, int &layer,
//	int &row, int &col, int nOctaveLayers, float contrastThreshold, float edgeThreshold, float sigma)
//static bool adjust_local_extrema(const std::vector<std::vector<cv::Mat>> &dogpyr, cv::KeyPoint &kpt, int octave, int layer, int r, int c, int nOctaveLayers, float contrastThreshold, float edgeThreshold, float sigma) {
//    float xr, xc, xi;
//    int iter = 0;
//    for (; iter < 5; iter++) {
...
//        r += cvRound(xr);
//        layer += cvRound(xi);
//        if (layer < 1 || layer > nOctaveLayers || c < 2 || c > curr.cols - 2 || r < 2 || r > curr.rows - 2) {
//            return false;
//        }
//    }
//    if (iter >= 5) {
			...
			for (int r = IMG_BORDER; r < num_row - IMG_BORDER; ++r)
			{
				...
				for (int c = IMG_BORDER; c < num_col - IMG_BORDER; ++c)
				{
						...
						val <= next_ptr[c + step - 1] && val <= next_ptr[c + step] && val <= next_ptr[c + step + 1])))
					{
						...
						float max_hist = calc_orientation_hist(gauss_pyr[octave][layer],
...
//	 vector<KeyPoint> &keypoints) const
//{
void MySift::detect(const cv::Mat &img, std::vector<std::vector<cv::Mat>> &gpyr, std::vector<std::vector<cv::Mat>> &dogpyr, std::vector<cv::KeyPoint> &keypoints) const {
    ...
    find_scale_space_extrema(dogpyr, gpyr, keypoints);
