...
#define GRAY1 0 // whether read image1 in gray scale
...
const double pyramid_ratio = 0.5;
...
void show_pyramid(cv::Mat &img, int levels, double ratio);
...
void show_pyramid(cv::Mat &img, int levels, double ratio) {
    ...
    for (int i = 0; i < levels; i++) {
        cv::Mat tmp;
        cv::resize(dst, tmp, cv::Size(), ratio, ratio);
        pyr.push_back(dst);
        dst = tmp;
    }
    cv::Mat dst;
    cv::resize(img, dst, cv::Size(), ratio, ratio);
    cv::imshow("pyramid", dst);
    cv::waitKey(0);
    if (levels > 1) {
        show_pyramid(dst, levels - 1, ratio);
    }
}