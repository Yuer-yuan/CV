...
def spectral_hybrid_image(image1, image2, d1, d2):
    ...
    plt.subplot(261), plt.imshow(image1, cmap='gray')
    plt.title('Input Image1'), plt.xticks([]), plt.yticks([])
    plt.subplot(262), plt.imshow(np.log1p(np.abs(f2shift)), cmap='gray')
    plt.title('Image1 Magnitude Spectrum'), plt.xticks([]), plt.yticks([])
    plt.subplot(263), plt.imshow(h1, cmap='gray')
    plt.title('Input Image2'), plt.xticks([]), plt.yticks([])
    plt.subplot(264), plt.imshow(np.log1p(np.abs(f1_filtered)), cmap='gray')
    plt.title('Image1 Low Pass Magnitude Spectrum'), plt.xticks([]), plt.yticks([])
    plt.subplot(265), plt.imshow(image2, cmap='gray')
    plt.title('Input Image2'), plt.xticks([]), plt.yticks([])
    plt.subplot(266), plt.imshow(np.log1p(np.abs(f2shift)), cmap='gray')
    plt.title('Image2 Magnitude Spectrum'), plt.xticks([]), plt.yticks([])
    plt.subplot(267), plt.imshow(h2, cmap='gray')
    plt.title('Input Image2'), plt.xticks([]), plt.yticks([])
    plt.subplot(268), plt.imshow(np.log1p(np.abs(f2_filtered)), cmap='gray')
    plt.title('Image2 High Pass Magnitude Spectrum'), plt.xticks([]), plt.yticks([])
    plt.subplot(269), plt.imshow(np.abs(f1_filtered_ifft), cmap='gray')
    plt.title(f'Image1 Low Pass. d1={d1}'), plt.xticks([]), plt.yticks([])
    plt.subplot(2610), plt.imshow(np.abs(f2_filtered_ifft), cmap='gray')
    plt.title(f'Image2 High Pass. d2={d2}'), plt.xticks([]), plt.yticks([])
    plt.subplot(2611), plt.imshow(hybrid_image, cmap='gray')
    plt.title('Hybrid Image'), plt.xticks([]), plt.yticks([])
    plt.show()
    return hybrid_image
