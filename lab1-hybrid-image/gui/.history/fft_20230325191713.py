...
if __name__ == '__main__':
    ...
    filter = ideal_lowpass_filter(img1.shape, 0.1)

    # Apply the filter to the FFTs.
    fftshift1_filtered = fftshift1 * filter
    fftshift2_filtered = fftshift2 * filter
    ...
    magnitude_spectrum1_filtered = 20*np.log(np.abs(ifftshift1))
    magnitude_spectrum2_filtered = 20*np.log(np.abs(ifftshift2))
