...
pairs = [
    ...
    ('res/dog.jpg', 'res/cat.jpg'),
    ('res/donkey1.jpg', 'res/zebra1.jpg'),
]
pair_idx = 2    # select a pair of images from the list above
...
def call_back_d1(x):
    ...
    show_pyramid_in_canvas(pyramid(hybrid_image, 5))
...
def show_pyramid_in_canvas(pyr):
    canvas = np.zeros((pyr[0].shape[0], np.sum([p.shape[1] for p in pyr])), dtype=np.uint8)
    x = 0
    for p in pyr:
...
if __name__ == '__main__':
    ...
    hybrid_image = spectral_hybrid_image(img1, img2, d1, d2)
    cv2.imshow('Display Exception', hybrid_image.astype(np.uint8))
    cv2.imshow('Display Normal', (hybrid_image / np.max(hybrid_image) * 255).astype(np.uint8))
    # cv2.imshow('Img1', img1)
    # cv2.imshow('Img2', img2)
    # cv2.imshow('Hybrid Image', hybrid_image)
    # show_pyramid_in_canvas(pyramid(hybrid_image, 5))
    # cv2.createTrackbar('d1', 'Hybrid Image', d1, 100, call_back_d1)
    # cv2.createTrackbar('d2', 'Hybrid Image', d2, 100, call_back_d2)
    cv2.waitKey(0)
