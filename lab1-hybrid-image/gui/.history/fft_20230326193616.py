...
def spectral_hybrid_image(image1, image2, d1, d2):
    ...
    magnitude1 = np.log1p(np.abs(f1shift))
    magnitude2 = np.log1p(np.abs(f2shift))

    plt.subplot(231), plt.imshow(image1, cmap='gray')
    plt.title('Input Image1'), plt.xticks([]), plt.yticks([])
    plt.subplot(232), plt.imshow(magnitude1, cmap='gray')
    plt.title('Image1 Magnitude Spectrum'), plt.xticks([]), plt.yticks([])
    plt.subplot(233), plt.imshow(image2, cmap='gray')
    plt.title('Input Image2'), plt.xticks([]), plt.yticks([])
    plt.subplot(234), plt.imshow(magnitude2, cmap='gray')
