...
def hybrid_image(img1, img2, ksize1, sigma1, ksize2, sigma2):
    ...
    filter2 = ideal_filter(img2.shape, 5, lowpass=False)
...
if __name__ == '__main__':
    ...
    cv2.createTrackbar('ksize1', 'Hybrid Image', ksize1, 100, lambda x: None)
    cv2.createTrackbar('sigma1', 'Hybrid Image', sigma1, 100, lambda x: None)
    cv2.createTrackbar('ksize2', 'Hybrid Image', ksize2, 100, lambda x: None)
    cv2.createTrackbar('sigma2', 'Hybrid Image', sigma2, 100, lambda x: None)

    while True:
        ksize1 = cv2.getTrackbarPos('ksize1', 'Hybrid Image')
        sigma1 = cv2.getTrackbarPos('sigma1', 'Hybrid Image')
        ksize2 = cv2.getTrackbarPos('ksize2', 'Hybrid Image')
        sigma2 = cv2.getTrackbarPos('sigma2', 'Hybrid Image')

        hybrid_image(img1, img2, ksize1, sigma1, ksize2, sigma2)
